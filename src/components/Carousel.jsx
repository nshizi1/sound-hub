      ...
      {images.map((image, index)=>{
        return (
          ...
          <div key={index} className={index=== current ? "carousel_card carousel_card-active" : "carousel_card"}>
            <img className='card_image' src={image.image} alt='imag' />
            <div className='card_overlay'>
              <h2 className='card_title'>{image.title}</h2>
              <p className='card_title_p'>{image.caption}</p>
              <a className='card_title_button' href='./Shop'><button>Shop Now</button></a>
            </div>
          </div>
        );
      })}
